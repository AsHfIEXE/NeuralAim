...
try {
    ...
    if ([string]::IsNullOrWhiteSpace($DestinationDir)) {
        $DestinationDir = Join-Path $RepoRoot "NeuralAim\modules\opencv\build\dml"
    }
    ...
    if ([string]::IsNullOrWhiteSpace($InstallerPath)) {
        $InstallerPath = Join-Path $RepoRoot ("NeuralAim\modules\_downloads\opencv-" + $OpenCvVersion + "-windows.exe")
    }
    ...
    $extractRoot = Join-Path (Join-Path $RepoRoot "NeuralAim\modules\_downloads") ("opencv-" + $OpenCvVersion + "-extract")
