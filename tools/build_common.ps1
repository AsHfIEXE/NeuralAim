...
function Get-ModulesDownloadCacheDir {
    return (Resolve-RepoPath 'NeuralAim\modules\_downloads')
}
...
function Restore-NuGetPackages {
    ...
    Invoke-External $nuget @(
        'restore', (Resolve-RepoPath 'NeuralAim\packages.config'),
...
function Test-CoreSourceModules {
    $serialRoot = Resolve-RepoPath 'NeuralAim\modules\serial'
    $simpleIni = Resolve-RepoPath 'NeuralAim\modules\SimpleIni.h'
...
function Install-SerialModule {
    ...
    if ($git) {
        Write-BuildStep "Cloning wjwwood/serial into NeuralAim\modules\serial" "modules"
        Invoke-External $git @(
            'clone',
            '--depth', '1',
...
function Ensure-CoreSourceModules {
    ...
    New-DirectoryIfMissing (Resolve-RepoPath 'NeuralAim\modules')

    if (-not $state.SimpleIniReady) {
        Invoke-DownloadFile `
            -Uri 'https://raw.githubusercontent.com/brofield/simpleini/master/SimpleIni.h' `
            -OutFile (Resolve-RepoPath 'NeuralAim\modules\SimpleIni.h') `
            -DryRun:$DryRun
    }
...
function Find-TensorRtRoot {
    $modulesDir = Resolve-RepoPath 'NeuralAim\modules'
