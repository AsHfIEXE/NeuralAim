...
try {
    ...
    if (-not $resolution.TensorRtRoot) {
        $downloadItems += New-DependencyDownloadItem `
            ...
            -Destination (Resolve-RepoPath "NeuralAim\modules") `
            -Action "extract"
    }
    ...
    if (-not $resolution.TensorRtRoot) {
        if ($DryRun) {
            $resolution.TensorRtRoot = Resolve-RepoPath "NeuralAim\modules\TensorRT-10.x"
        }
        else {
    ...
    $opencvCudaInstall = Resolve-RepoPath "NeuralAim\modules\opencv\build\cuda\install"
